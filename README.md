...
- `src/game/` — **motor puro** (sem canvas/DOM, testável): `constants` (números ajustáveis),
  ...
  sorteio de tipo), `upgrades` (ajusta ataque/cadência/alcance do núcleo, com limites), `particles`
  (explosão de partículas na morte de inimigos/dano no núcleo), `step` (orquestra um frame).
